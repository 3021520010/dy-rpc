...
public class Client {
    ...
    public <T> T getProxy(Class<T> clazz,int retryCount, int retryTime) {
        return (T) Proxy.newProxyInstance(clazz.getClassLoader(),
                new Class[]{clazz},
                new RemoteInvoker(clazz, encoder, decoder, serviceRegistry, loadBalancer,retryCount,retryTime,null));
    }
    public <T> T getProxy(Class<T> clazz,int retryCount, int retryTime,LoadBalancer loadBalancer,String key) {
        return (T) Proxy.newProxyInstance(clazz.getClassLoader(),
                new Class[]{clazz},
                new RemoteInvoker(clazz, encoder, decoder, serviceRegistry, loadBalancer,retryCount,retryTime,key));
    }
